...
export default ({ getEntity, getError, getIsConnecting, hasQueryCache, readEntity }) => (
  (WrappedComponent) => {
    ...
    const mapStateToProps = (state, { getQuery, id }) => ({
      entity: hasQueryCache(state, getQuery(id)) ? getEntity(state, getQuery(id)) : undefined,
